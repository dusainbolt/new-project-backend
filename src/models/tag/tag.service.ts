...
export class TagService {
  ...
  //   // update Tag
  //   const tagDataUpdate = await this.tagModel.findByIdAndUpdate(tagId, {
  //     ...data,
  //     slug,
  //   });
  //   // Return result
  ...
  //       .limit(limit)
  //       .sort({ createdAt: Constant.find.DESC });
  //   } else {
  //     return this.tagModel
  //       .find({ status: { $in: status } })
  //       .sort({ createdAt: Constant.find.DESC });
  ...
  //   // Query total
  //   const total = searchTagInput.count
  //     ? await this.tagModel.countDocuments(query as any)
  //     : 0;
  //   // Return result
